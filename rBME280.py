...
def weatherSense(startDateTime, hostIP, BASE_PORT, streaming=True, logging=True):
    ...
    #for HEARTBEAT_LOG
    weatherHeartbeatFileName = BASE_PATH+"Relay_Station{0}/heartbeatLog/Weather.txt".format(BASE_PORT)
    with open(weatherHeartbeatFileName, "a") as weatherHeartbeat:
        weatherHeartbeat.write("sensorType, timeStamp, value\n")
    #for HEARTBEAT_LOG
...
#            weatherMessage.append(str(sumTemp))
            ...
            #for HEARTBEAT_LOG
            with open(weatherHeartbeatFileName, "a") as weatherHeartbeat:
                weatherHeartbeat.write("Humidity,{0},{1}\n".format(startDateTime,weatherMessage[1]))
            #for HEARTBEAT_LOG
